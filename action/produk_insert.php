...
if($_FILES["path_produk"]["name"]){
				...
				if($emptyData==0){
					if($_FILES["path_produk"]["size"]>3000000){
						...
						if(move_uploaded_file($_FILES["path_produk"]["tmp_name"], $gambar["target"])) {
						 	 $product->createProduct($_POST["kategori"], $_POST["nama_produk"], $gambar["target"], $_POST["keterangan"]);
			...
			// alert if field not an image or not empty
	        ?>
			<div class="alert alert-danger" >File gambar kosong.</div>
			<?php
		}
?>